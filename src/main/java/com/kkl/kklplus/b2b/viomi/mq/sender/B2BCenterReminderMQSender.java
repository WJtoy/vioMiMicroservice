...
@Slf4j
@Component
public class B2BCenterReminderMQSender implements RabbitTemplate.ConfirmCallback {
    ...
    @Autowired
    public B2BCenterReminderMQSender(RabbitTemplate kklRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
     ...
     * @param message 消息体
     */
    public void send(MQB2BOrderReminderMessage.B2BOrderReminderMessage message) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                rabbitTemplate.convertAndSend(
                        B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER,
                        B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER,
                        message.toByteArray(),
                        new CorrelationData());
                return null;
                ...
                log.error("MQ_B2BCENTER_NEW_B2BORDER_REMINDER {}, {}", throwable.getLocalizedMessage(), msg);
                String msgJson = new JsonFormat().printToString(msg);
                sysLogService.insert(1L,msgJson,"调用自动催单队列失败：" + throwable.getLocalizedMessage(),
                        "调用自动催单队列失败", B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER, "");
                return null;
            });
        } catch (Exception e) {
            String msgJson = new JsonFormat().printToString(message);
            sysLogService.insert(1L,msgJson,"调用自动催单队列失败：" + e.getMessage(),
                    "调用自动催单队列失败", B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER, "");
        }
